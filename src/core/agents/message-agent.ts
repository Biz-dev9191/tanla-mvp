...
/**
 * Ensures strict punctuation, grammar, and capitalization:
 * - Every line / paragraph begins with a capital letter
 * - Sentences following periods or question marks begin with a capital letter
 * - List bullet points begin with a capital letter
 * - Collapses excessive blank lines
 */
export function formatChannelMessage(text: string): string {
  if (!text) return '';
  ...
  const formattedLines = lines.map((line) => {
    ...
    if (bulletMatch) {
      ...
      const capitalizedRest = rest ? rest.charAt(0).toUpperCase() + rest.slice(1) : '';
      return `${marker} ${capitalizedRest}`;
    }
...
/**
 * Assembles WhatsApp messages with clean paragraph breaks (\n\n)
 * preventing any comma splices after salutations and guaranteeing proper paragraph capitalization.
 */
export function assembleWhatsApp(
  greeting: string,
  frustrationPrefix: string,
  vipPrefix: string,
  body: string
): string {
  const parts: string[] = [greeting];
  if (frustrationPrefix.trim()) parts.push(frustrationPrefix.trim());
  if (vipPrefix.trim()) parts.push(vipPrefix.trim());
...
export function runMessageGenerationAgent(
  ...
  // Demographic & Generational Persona Identification
  if (persona.cohort === 'Gen Z (18–26)') {
    chainOfThought.push(
      `Identified Customer demographic cohort: Gen Z (18–26). Digital-first mobile channel prioritized. Tone: Casual-competent, active voice, zero fluff.`
    );
  } else if (persona.cohort === 'Baby Boomer (59–77)' || persona.cohort === 'Silent Generation (78+)') {
    chainOfThought.push(
      `Identified Customer demographic cohort: Senior/Boomer (${persona.cohort}). Tone: Reassuring, patient, step-by-step clarity, affirming security.`
    );
  } else {
    chainOfThought.push(
      `Identified Customer demographic cohort: ${persona.cohort}. Tone: Balanced, efficient, outcome-first.`
    );
  }
  ...
  const voiceGreeting = isVIP
    ? `Hello ${cleanFullName}, this is an executive priority update.`
    : isSenior
    ? `Hello ${cleanFullName}, this is an important update.`
    : isGenZ
    ? `Hi ${firstName}, quick update.`
    : `Hello ${firstName}, this is an automated update.`;
  ...
  const vipWaPrefix = isVIP ? 'As a valued VIP member, your request has been prioritized.' : '';
  ...
  if (event.eventType === 'payment_successful_order_failed') {
    ...
    // 2. WhatsApp: Persona-customized with clean paragraph structure
    if (isGenZ) {
      waText = assembleWhatsApp(
        waGreeting,
        frustrationWaPrefix,
        vipWaPrefix,
        `Your payment for ${orderRef}${payRef} was received, but the order failed due to inventory. ${refundAmountRef.charAt(0).toUpperCase() + refundAmountRef.slice(1)} has been initiated to your original payment method. No action needed on your end.\n\nTrack status live in your app and web dashboard.${customGoodwillSentence ? `\n\n${customGoodwillSentence}` : ''}`
      );
    } else if (isSenior) {
      waText = assembleWhatsApp(
        waGreeting,
        frustrationWaPrefix,
        vipWaPrefix,
        `We are contacting you regarding ${orderRef}. Please be assured that your payment${payRef} is completely safe, and we have automatically initiated ${refundAmountRef} back to your original payment method.\n\nYou do not need to take any action. The funds will reflect in your account within 3 to 5 business days.${customGoodwillSentence ? `\n\n${customGoodwillSentence}` : ''}\n\nFor real-time confirmation, please check your app and web dashboard.\n\nWarm regards,\nCustomer Support Team`
      );
    } else {
      waText = assembleWhatsApp(
        waGreeting,
  ...
  // =========================================================================
    ...
    if (isGenZ) {
      waText = assembleWhatsApp(
        ...
        frustrationWaPrefix,
        vipWaPrefix,
        `Heads up that ${orderRef} is running slightly behind schedule due to ${delayReason}. We are prioritizing delivery and expect it to arrive within 2 to 3 business days.\n\nTrack package live in your app and web dashboard.${customGoodwillSentence ? `\n\n${customGoodwillSentence}` : ''}\n\nNo action needed on your end.`
      );
    } else if (isSenior) {
      waText = assembleWhatsApp(
        waGreeting,
    ...
    if (isGenZ) {
      ...
      voiceScript = `${voiceGreeting} Regarding ${orderRef}, due to transit conditions, your delivery has been rescheduled. Our team is closely monitoring your shipment to ensure safe arrival. You can check the updated delivery status in your app and web dashboard. No action is required from you. Thank you for your patience.`;
    } else {
  ...
  // =========================================================================
    ...
    if (isGenZ) {
      ...
      voiceScript = `${voiceGreeting} Our team is conducting scheduled system maintenance. Please be assured that your account, data, and security remain fully protected. Live status is available in your app and web dashboard. Thank you for your understanding.`;
    } else {
      voiceScript = `${voiceGreeting} Scheduled system maintenance is in progress. All customer accounts and data are secure. For live updates, please check your app and web dashboard. Thank you.`;
    }
  ...
  // =========================================================================
    ...
    if (isGenZ) {
      waText = assembleWhatsApp(
        ...
        frustrationWaPrefix,
        vipWaPrefix,
        `Heads up that your ${planName} renews shortly${hasAmount ? ` (${event.amount})` : ''}. Review and confirm your renewal details in your app or web dashboard in 1 tap.${customGoodwillSentence ? `\n\n${customGoodwillSentence}` : ''}\n\nOur team is available in your dashboard if you have any questions.`
      );
    } else if (isSenior) {
      waText = assembleWhatsApp(
        waGreeting,
      ...
      waText = assembleWhatsApp(
        ...
        frustrationWaPrefix,
        vipWaPrefix,
        `This is a friendly reminder that your ${planName} is scheduled for renewal shortly${hasAmount ? ` (${event.amount})` : ''}.\n\nTo ensure continuous access, please review and confirm your renewal details in your app or web dashboard.${customGoodwillSentence ? `\n\n${customGoodwillSentence}` : ''}\n\nOur team is available if you have any questions.`
      );
    }
    ...
    if (isGenZ) {
      ...
      voiceScript = `${voiceGreeting} This is a courtesy reminder that your subscription is scheduled for renewal soon. To review your plan or update payment details, please log in to your app and web dashboard. Thank you.`;
    }
  ...
  // =========================================================================
    ...
    if (isGenZ) {
      waText = assembleWhatsApp(
        ...
        frustrationWaPrefix,
        vipWaPrefix,
        `Heads up that your payment${amountRef ? ` of ${amountRef}` : ''} for ${orderRef} using ${declineCard} was declined by the bank. To keep your order active and prevent cancellation, please log in to your app or web dashboard to securely retry within 24 hours. No re-registration needed.`
      );
    } else if (isSenior) {
      waText = assembleWhatsApp(
        waGreeting,
    ...
    if (isGenZ) {
      ...
      voiceScript = `${voiceGreeting} The bank authorization for ${orderRef} was not successful. To ensure your order remains active, please visit your app and web dashboard to securely review your payment details within 24 hours. Our support team is available in your dashboard if you need assistance. Thank you.`;
    } else {
  ...
  // =========================================================================
    ...
    if (isGenZ) {
      waText = assembleWhatsApp(
        ...
        frustrationWaPrefix,
        vipWaPrefix,
        `Your initial application review${appIdText} is approved. To finalize your account setup, please upload your ${missingDoc} via your app or web dashboard by the stated deadline. Zero hassle.`
      );
    } else if (isSenior) {
      waText = assembleWhatsApp(
        waGreeting,
      ...
      waText = assembleWhatsApp(
        ...
        frustrationWaPrefix,
        vipWaPrefix,
        `Thank you for submitting your application${appIdText}. Your initial identity verification has been approved. To finalize account activation, please upload your ${missingDoc} through your app or web dashboard by the stated deadline.\n\nOur team is available in your dashboard if you need any assistance.`
      );
    }
  ...
  // =========================================================================
    ...
    if (isGenZ) {
      ...
      voiceScript = `${voiceGreeting} Regarding your recent inquiry on ${orderRef}, we have prioritized your case and our operations supervisor will provide a full resolution within 4 business hours. Thank you.`;
    }
  ...
  // =========================================================================
    ...
    if (isGenZ) {
      ...
      voiceScript = `${voiceGreeting} Regarding ${event.title.toLowerCase()}, ${strategy.customerActionRequired ? 'please check your dashboard.' : 'no action is required.'} Thank you.`;
    }

    // 4. Email: Governed by gravity
    emailSubject = `Important update regarding your account: ${event.title}`;
    if (isCritical) {
      emailBody = `Dear ${cleanFullName},\n\nWe are writing to provide you with a formal update regarding your account.\n\nSummary:\n${event.description}\n\nVerified details:\n- ${factsSummary}\n${customGoodwillSentence ? `\nResolution:\n${customGoodwillSentence}\n` : ''}\nNext steps:\n${strategy.customerActionRequired ? 'Please log in to your account dashboard to review the required steps.' : 'This update is purely informational and no action is required on your part.'}\n\nSincerely,\nCompliance & Operations Team`;
    } else {
      emailBody = `${emailGreeting}\n\nWe are writing to provide you with an update regarding your account.\n\nSummary:\n${event.description}\n\nVerified details:\n- ${factsSummary}\n${customGoodwillSentence ? `\nResolution:\n${customGoodwillSentence}\n` : ''}\nNext steps:\n${strategy.customerActionRequired ? 'Please log in to your account dashboard to review the required steps.' : 'This update is purely informational and no action is required on your part.'}\n\n${emailClosing}`;
    }
  }
  ...
  // Universal formatting pass: Strict capitalization, punctuation, and grammar across all channels
  waText = formatChannelMessage(waText);
  smsText = formatChannelMessage(smsText);
  emailBody = formatChannelMessage(emailBody);
  voiceScript = formatChannelMessage(voiceScript);
